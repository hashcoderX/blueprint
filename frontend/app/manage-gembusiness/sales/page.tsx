...
export default function GemSales() {
  ...
  const handleAddSale = async () => {
    ...
    try {
      ...
      setDescription('');
      setShowAdd(false);
      // Go back to Inventory so the user sees the item marked as sold
                ...
                <p className="text-3xl font-bold">
                  {totalSales > 0
                    ? new Intl.NumberFormat(undefined, { style: 'currency', currency: userProfile?.currency || 'USD' }).format(
                        sales.reduce((s, p) => s + (Number(p.amount) || 0), 0) / totalSales
                      )
                    : '$0.00'
                  }
                </p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="bg-white dark:bg-powerbi-gray-800 divide-y divide-powerbi-gray-200 dark:divide-powerbi-gray-700">
                  {sales.map(s => (
                    <tr key={s.id} className="hover:bg-powerbi-gray-50 dark:hover:bg-powerbi-gray-900/50">
